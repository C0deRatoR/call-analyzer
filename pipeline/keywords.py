...
def _fallback_tfidf_keywords(text: str, top_n: int = 10) -> list[tuple[str, float]]:
    ...
    keywords = [(word, round(count / max_count, 4)) for word, count in counts.most_common(top_n)]
    return keywords
...
def extract_keywords(
    ...
    if model is not None:
        try:
            ...
            result_keywords = [
                {"keyword": kw, "score": round(score, 4)} for kw, score in raw_keywords
            ]
        except Exception as e:
    ...
    if model is None or not result_keywords:
        ...
        result_keywords = [{"keyword": kw, "score": score} for kw, score in fallback]
